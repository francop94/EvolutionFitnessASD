...
public class NOTIFICATION_Thread_DayCert extends Application implements Runnable{
        ...
        public NOTIFICATION_Thread_DayCert (NotificationChannel chan, NotificationManager man, NotificationCompat.Builder build,PendingIntent pending) {
        ...
        public void run(){
            ...
            while (!Thread.currentThread().isInterrupted()&&!sendedCert) {
                try {
                    ...
                    System.out.println(certMed);
                    if(certMed!=null){
                        try {
                            newCertDate= sdf.parse(certMed);
                            System.out.println("CERT MED: "+certMed);
